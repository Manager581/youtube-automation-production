...
STAGES = [
    ...
    # PHASE 5: EDITORIAL — paper edit workflow
    ("selects",         "Tag selects (best moments per clip)",  "pipeline_v2/selects_tagger.py"),
    ("paper_edit",      "Generate paper edit (horizontal board)", "pipeline_v2/paper_edit_generator.py"),
    ("edit_review",     "Review paper edit (exec producer) [GATE]", "pipeline_v2/edit_reviewer.py"),
    ("edit_approve",    "Approve paper edit (interactive)",     "pipeline_v2/edit_approve.py"),
...
def build_stage_args(stage_name, state):
    ...
    args_map = {
        ...
        "voice": ["--ref-audio", "assets/voice/voice_neutral_ref_short.wav",
                  ...
                  "--out", config.get("narration_path", "audio/narration.wav")],
        ...
        "verify_footage": [],
        "selects": ["--transcripts", config.get("transcript_path", f"media/{slug}/transcripts.json"),
                    "--vision", config.get("vision_manifest", f"media/{slug}/vision_manifest.json"),
                    "--output", config.get("selects_path", f"storyboards/{slug}_selects.json")],
        "paper_edit": ["--script", config.get("script_path", "scripts/enhanced_script.txt"),
                       "--alignment", config.get("alignment_path", "audio/narration_alignment.json"),
                       "--selects", config.get("selects_path", f"storyboards/{slug}_selects.json"),
                       "--output-json", config.get("paper_edit_path", f"storyboards/{slug}_paper_edit.json"),
                       "--output-md", config.get("paper_edit_md_path", f"storyboards/{slug}_paper_edit.md")],
        "edit_review": ["--paper-edit", config.get("paper_edit_path", f"storyboards/{slug}_paper_edit.json"),
                        "--selects", config.get("selects_path", f"storyboards/{slug}_selects.json"),
                        "--output", config.get("paper_edit_reviewed_path", f"storyboards/{slug}_paper_edit_reviewed.json")],
        "edit_approve": ["--reviewed", config.get("paper_edit_reviewed_path", f"storyboards/{slug}_paper_edit_reviewed.json"),
                         "--output", config.get("paper_edit_approved_path", f"storyboards/{slug}_paper_edit_approved.json"),
                         "--auto-approve", "0.85"],
        ...
        "davinci_build": ["--output", config.get("fcpxml_path", "timeline_final.fcpxml"),
                           "--paper-edit", config.get("paper_edit_approved_path", f"storyboards/{slug}_paper_edit_approved.json"),
                           "--import-resolve"],
